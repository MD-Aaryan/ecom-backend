...
export class ProductService {
  ...
  async create(dto: CreateProductDto, file?: UploadedFile) {
    let imageUrl = dto.imageUrl ?? 'https://via.placeholder.com/600';
    if (file) {
  ...
  async searchAndFilter(query: ProductQueryDto) {
    ...
    if (query.ids) {
      const idList = query.ids
        ...
        .filter(Boolean);
      if (idList.length > 0) {
        const placeholders = idList.map(() => `$${++paramIdx}`).join(',');
    ...
    const [rows, countResult] = await Promise.all([
      this.prisma.$queryRawUnsafe<any[]>(
        `SELECT p.id, p.title, p.description, p.price, p."imageUrl", p.stock, p."isActive", p."createdAt", p."updatedAt", p."categoryId", p."subcategoryId", c.name AS "categoryName", (SELECT AVG(r."rating")::float FROM "Review" r WHERE r."productId" = p.id) AS "avgRating", (SELECT COUNT(*)::int FROM "Review" r WHERE r."productId" = p.id) AS "reviewCount" FROM "Product" p LEFT JOIN "Category" c ON c.id = p."categoryId" WHERE ${whereClause} ORDER BY ${orderClause} LIMIT $${++paramIdx} OFFSET $${++paramIdx}`,
        ...params,
        take,
        skip,
      ),
