...
export class ReviewController {
  ...
  getProductReviews(@Param('id') id: string, @Query('page') page?: string, @Query('limit') limit?: string) {
    return this.reviewService.getProductReviews(+id, page ? +page : undefined, limit ? +limit : undefined);
  }

  @Post('products/:id/reviews')
  @UseGuards(JwtAuthGuard)
  create(@Param('id') id: string, @Body() dto: CreateReviewDto, @Req() req: any) {
