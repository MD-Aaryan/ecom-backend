...
export class OrderService {
  ...
  async placeOrder(userId: number, dto: CreateOrderDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      const [products, variants] = await Promise.all([
        ...
        variantIds.length ? tx.productVariant.findMany({ where: { id: { in: variantIds } } }) : [],
      ]);

      const productMap = new Map(products.map((p): [number, typeof p] => [p.id, p]));
      const variantMap = new Map(variants.map((v): [number, typeof v] => [v.id, v]));

      let total = 0;
      const itemsWithPrice: (typeof cart.items[0] & { price: number })[] = cart.items.map((item) => {
        const product = productMap.get(item.productId);
        if (!product || !product.isActive) throw new BadRequestException(`Product ${item.productId} unavailable`);
        if (item.quantity > product.stock) throw new BadRequestException(`Insufficient stock for ${product.title}`);
      ...
      if (dto.couponCode) {
        ...
        if (total < coupon.minOrderAmount) throw new BadRequestException('Minimum order amount not met');
  ...
  async cancelOrder(orderId: number, userId: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!order) throw new NotFoundException('Order not found');
      if (order.userId !== userId) throw new ForbiddenException('Not your order');
      if (!['PENDING', 'APPROVED'].includes(order.status)) throw new BadRequestException('Order cannot be cancelled');
  ...
  async getAllOrders(page?: number, limit?: number) {
    ...
    const [data, total] = await Promise.all([
      this.prisma.order.findMany({
        ...
        take,
        orderBy: { createdAt: 'desc' },
        include: { user: { select: { id: true, name: true, email: true } }, items: { include: { product: true } }, payment: true },
      }),
