...
export class CategoryService {
  ...
  async create(dto: CreateCategoryDto, adminId: string) {
    ...
    await this.auditLog.logAction('CREATE', 'Category', category.id, adminId, null, { name: category.name });
    return category;
  }
  ...
  async update(id: string, dto: UpdateCategoryDto, adminId: string) {
    ...
    await this.cache.del('categories:all');
    await this.auditLog.logAction('UPDATE', 'Category', id, adminId, { name: oldCategory.name }, { name: updated.name });
    return updated;
  }
  ...
  async softDelete(id: string, adminId: string) {
    ...
    await this.cache.del('categories:all');
    await this.auditLog.logAction('DELETE', 'Category', id, adminId, { name: oldCategory.name }, null);
