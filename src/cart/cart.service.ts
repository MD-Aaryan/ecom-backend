...
export class CartService {
  ...
  async addToCart(userId: number, dto: AddToCartDto) {
    ...
    if (dto.quantity > stock) throw new BadRequestException('Insufficient stock');
    ...
    return this.prisma.cartItem.create({ data: { cartId: cart.id, productId: dto.productId, variantId: dto.variantId, quantity: dto.quantity } });
  }
  ...
  async removeItem(userId: number, cartItemId: number) {
    const cart = await this.getOrCreateCart(userId);
    if (!cart.items.find((i) => i.id === cartItemId)) throw new NotFoundException('Cart item not found');
