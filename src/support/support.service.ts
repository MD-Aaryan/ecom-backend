...
export class SupportService {
  ...
  async replyToTicket(
    ...
    // Admin reply → notify ticket owner
    if (isAdmin) {
  ...
  async adminCloseTicket(ticketId: string) {
    const ticket = await this.prisma.supportTicket.findUnique({
      where: { id: ticketId },
    });
    if (!ticket) throw new NotFoundException('Ticket not found');
  ...
  async removeTicket(ticketId: string) {
    ...
    await this.prisma.$transaction([
      this.prisma.ticketReply.deleteMany({ where: { ticketId } }),
      this.prisma.supportTicket.delete({ where: { id: ticketId } }),
    ]);
    return { success: true };
  }
}
