...
async function main() {
  ...
  for (const email of ['hobbyist1@example.com', 'hobbyist2@example.com', 'hobbyist3@example.com', 'hobbyist4@example.com', 'hobbyist5@example.com']) {
    await prisma.newsletterSubscriber.create({ data: { email, isActive: true } });
  }
  console.log('Newsletter subscribers seeded: 5');
  ...
  const webhook = await prisma.webhook.create({
    data: {
      ...
      logs: {
        create: [
          { event: 'order.created', status: 200, response: '{"ok":true}', success: true },
          { event: 'payment.success', status: 500, response: 'timeout', success: false },
        ],
      },
    },
  });
  console.log(`Webhook seeded: ${webhook.url}`);
